...
def compute_timeliness(window_length_sec: int, desired_timeliness_sec: float) -> float:
    global observations
    obs_list = list(observations)
    obs_required = obs_list[-window_length_sec:]

    count = 0
    for obs in obs_required:
        if obs <= desired_timeliness_sec:
            count += 1

    percentile = float(count/window_length_sec)
...
def compute_odte_phytodig(window_length_sec, desired_timeliness_sec, expected_msg_sec):
    timeliness = compute_timeliness(window_length_sec, desired_timeliness_sec)
